...
//          Driver in C to show information on LCD
//*****************************************************************************
...
//void LCD_Display_Result(uint16_t);
...
/*int main(void) {
	...
	LCD_Configure();
	LCD_Display_Result( )//needs argument
...
//void LCD_Display_Result(uint16_t r) {
	/*	int8_t i,j=2, count;
	  uint8_t dig[4];
	  uint8_t ch, color[2];
		...
		if (count > 10)
			//myLCD3(2);
			LCD_WriteChar(&color[0],0,0,3);
		else
			//myLCD3(1);
			LCD_WriteChar(&color[1],0,0,3);
		...
		Delay(50);
		if (count > 10)
			LCD_WriteChar(&color[0],0,0,4);
		else
			LCD_WriteChar(&color[1],0,0,4);
		...
		Delay(50);
		if (count > 10)
			LCD_WriteChar(&color[0],0,0,5);
		else
			LCD_WriteChar(&color[1],0,0,5);
		...
		Delay(50);
		if (count > 10)
			LCD_WriteChar(&color[0],0,0,6);
		else
			LCD_WriteChar(&color[1],0,0,6);
